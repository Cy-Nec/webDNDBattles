...
export const getUniqueInitiatives = (participants) => {
  return [...new Set(participants.map(p => p.initiative))].sort((a, b) => b - a)
}

// Определение текущего хода
export const getCurrentTurnParticipants = (sortedParticipants, uniqueInitiatives, currentTurnIndex) => {
  const currentInitiative = uniqueInitiatives[currentTurnIndex] !== undefined
...
// Проверка, является ли участник частью текущего хода
...
export const getParticipantCardClass = (participant, isCombat = false, isCurrent = false) => {
